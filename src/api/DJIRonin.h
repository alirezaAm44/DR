...
private:
    ...
    // -----------------------------------------------------------------------
    // RX multi-frame reassembly state
    // -----------------------------------------------------------------------

    PacketBuffer rxPacket_;

    // Expected total DJI packet length extracted from Ver/Length.
    // 0 means that the packet header has not been completely received yet.
    uint16_t rxExpectedLength_ = 0;

    // True while one DJI packet is being assembled from multiple CAN frames.
    bool rxReceiving_ = false;
